...
def category_frequency_count(category):

    category_train = fetch_20newsgroups(subset='train', categories=category, shuffle=True, random_state=42)
    frequency = []
    frequency = collections.Counter(category_train.target)
    docs_count = sum(frequency.values())
    return docs_count
...
comp_tech_freq = category_frequency_count(comp_tech_categories)
print comp_tech_freq

# get frequency of docs in Recreational category
rec_categories = ['rec.autos', 'rec.motorcycles', 'rec.sport.baseball', 'rec.sport.hockey']
rec_freq = category_frequency_count(rec_categories)
print rec_freq
